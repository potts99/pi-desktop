...
const api: Api = {
	...
	listWorkspaceFiles: (cwd, prefix) =>
		ipcRenderer.invoke("listWorkspaceFiles", cwd, prefix),
	listSlashCommands: (cwd) => ipcRenderer.invoke("listSlashCommands", cwd),
	listGitBranches: (cwd) => ipcRenderer.invoke("listGitBranches", cwd),
	checkoutGitBranch: (cwd, branch) =>
		ipcRenderer.invoke("checkoutGitBranch", cwd, branch),
	...
	respondToUiRequest: (k, response) =>
		ipcRenderer.invoke("respondToUiRequest", k, response),
	getModels: (k) => ipcRenderer.invoke("getModels", k),
	getSharedModels: (sessionKey) =>
		ipcRenderer.invoke("getSharedModels", sessionKey),
	...
	setThinkingLevel: (k, level) =>
		ipcRenderer.invoke("setThinkingLevel", k, level),
	...
	getMetricsSummary: (filter) =>
		ipcRenderer.invoke("getMetricsSummary", filter),
	refreshMetricsBackfill: (filter) =>
		ipcRenderer.invoke("refreshMetricsBackfill", filter),
	getPinned: () => ipcRenderer.invoke("getPinned"),
	togglePin: (path) => ipcRenderer.invoke("togglePin", path),
	getSettings: () => ipcRenderer.invoke("getSettings"),
	getAdvisorConfig: () => ipcRenderer.invoke("getAdvisorConfig"),
	updateAdvisorConfig: (partial) =>
		ipcRenderer.invoke("updateAdvisorConfig", partial),
	getSystemPrompt: () => ipcRenderer.invoke("getSystemPrompt"),
	updateSystemPrompt: (systemPrompt) =>
		ipcRenderer.invoke("updateSystemPrompt", systemPrompt),
