...
function mergeMessage(
	...
	if (next.id) {
		...
		if (
			...
			!last.id &&
			last.role === next.role &&
			messageText(last) === messageText(next)
		) {
...
export function useSessions() {
	...
	// ponytail: a new thread has no session yet, so its picker selections are held here
	// and applied to the real session at open time (see send). Reset on each new thread.
	const [newThreadModel, setNewThreadModel] = useState<ModelChoice | null>(
		null,
	);
	const [newThreadThinking, setNewThreadThinking] =
		useState<ThinkingLevel>("medium");
	const [globalError, setGlobalError] = useState<string | null>(null);
	const [advisorModel, setAdvisorModel] = useState<ModelChoice | null>(null);
	...
	const startedEntryRef = useRef(false);
	...
	useEffect(
		() => window.pi?.onSessionsChanged(() => refreshWorkspaces()),
		[refreshWorkspaces],
	);
	...
	useEffect(() => {
		...
		// ponytail: advisor config is global, fetch once to label the reviewing indicator
		window.pi?.getAdvisorConfig?.().then((c) => {
			if (!cancelled) setAdvisorModel(c.model);
		});
		return () => {
			cancelled = true;
		};
		...
		() =>
			window.pi?.onSessionEvent((key, ev) => {
				if (ev.kind === "message") {
					patchTab(key, (t) => ({
						...t,
						messages: mergeMessage(t.messages, ev.message),
						streamingText:
							ev.message.role === "assistant" ? "" : t.streamingText,
					}));
					refreshStats(key);
					...
					patchTab(key, (t) => ({
						...t,
						streaming: true,
						streamingText: t.streamingText + ev.text,
						workingStartedAt: t.workingStartedAt ?? Date.now(),
					}));
					...
					refreshStats(key);
				} else if (ev.kind === "advisory") {
					patchTab(key, (t) => ({ ...t, advisorReviewing: ev.reviewing }));
				} else if (ev.kind === "queue") {
					patchTab(key, (t) => ({ ...t, queue: ev.queue }));
				} else if (ev.kind === "retry") {
					patchTab(key, (t) => ({ ...t, retry: ev.retry }));
				} else if (ev.kind === "uiRequest") {
					patchTab(key, (t) => ({
						...t,
						uiRequests: [
							...t.uiRequests.filter((r) => r.id !== ev.request.id),
							ev.request,
						],
					}));
					...
					patchTab(key, (t) => ({
						...t,
						sessionPath:
							ev.state.sessionPath !== undefined
								? (ev.state.sessionPath ?? null)
								: t.sessionPath,
						...
						activeModel:
							ev.state.model !== undefined
								? (ev.state.model ?? null)
								: t.activeModel,
						workingStartedAt:
							ev.state.workingStartedAt !== undefined
								? ev.state.workingStartedAt
								: t.workingStartedAt,
					}));
	...
	function nextTab() {
		setActiveIdx((prev) =>
			tabsRef.current.length > 0 ? (prev + 1) % tabsRef.current.length : -1,
		);
	}
	...
	const openSession = useCallback(
		async (arg: { path: string } | { newIn: string }) => {
			...
			const existing = tabsRef.current.findIndex(
				(t) => t.sessionPath === sessionPath && sessionPath !== null,
			);
			if (existing >= 0) {
				setNewThread(false);
			...
			try {
				const cwd =
					"newIn" in arg
						? arg.newIn
						: (groups.find((g) => g.sessions.some((s) => s.path === arg.path))
								?.path ?? null);
				const openArg =
					"newIn" in arg ? arg : { ...arg, cwd: cwd ?? undefined };
				const {
					sessionKey,
					messages: history,
					state,
				} = await window.pi.openSession(openArg);
				...
				tab.workingStartedAt = state.workingStartedAt ?? null;
				...
				void window.pi
					.getModels(sessionKey)
					.then((m) => patchTab(sessionKey, (t) => ({ ...t, models: m })))
					.catch(() => {});
	...
	const send = useCallback(
		async (text: string, mode: "prompt" | "steer" | "followUp" = "prompt") => {
			if (!window.pi) return;
			...
			if (newThread) {
				...
				try {
					...
					if (sessionKey) {
						if (newThreadModel) {
							await window.pi.setModel(
								sessionKey,
								newThreadModel.provider,
								newThreadModel.id,
							);
							patchTab(sessionKey, (tab) => ({
								...tab,
								activeModel: newThreadModel,
							}));
						}
						if (newThreadThinking !== "medium")
							await window.pi.setThinkingLevel(sessionKey, newThreadThinking);
						await window.pi.sendPrompt(sessionKey, text, "prompt");
					}
				} catch (err) {
					setOpening(false);
	...
	const queuePending = useCallback((message: string) => {
		const key = tabsRef.current[activeIdxRef.current]?.sessionKey;
		if (!key) return;
		patchTab(key, (t) => ({ ...t, pending: [...t.pending, message] }));
	}, []);

	const pushPending = useCallback(
		(index: number) => {
			const tab = tabsRef.current[activeIdxRef.current];
			const message = tab?.pending[index];
			if (!tab || message === undefined) return;
			patchTab(tab.sessionKey, (t) => ({
				...t,
				pending: t.pending.filter((_, i) => i !== index),
			}));
			void send(message, tab.streaming ? "steer" : "prompt");
		},
		[send],
	);
	...
	const cycleThinking = useCallback(async () => {
		if (newThread) {
			setNewThreadThinking(
				(prev) =>
					thinkingLevels[
						(thinkingLevels.indexOf(prev) + 1) % thinkingLevels.length
					],
			);
			return;
		}
	...
	const startNewThread = useCallback((cwd: string | null) => {
		setNewThreadCwd(cwd);
		setNewThreadError(null);
		setNewThreadModel(null);
		setNewThreadThinking("medium");
		setNewThread(true);
	}, []);

	useEffect(() => {
		if (
			startedEntryRef.current ||
			!workspacesLoaded ||
			tabsRef.current.length > 0
		)
			return;
		startedEntryRef.current = true;
		startNewThread(groups[0]?.path ?? null);
	}, [groups, startNewThread, workspacesLoaded]);
	...
	const workingStartedAt = tab?.workingStartedAt ?? null;
	const advisorReviewing = tab?.advisorReviewing ?? false;
	const discoveredModels = tabs.flatMap((openTab) => openTab.models);
	const models = tab
		? tab.models.length > 0
			? tab.models
			: defaultModels
		: [...defaultModels, ...discoveredModels].filter(
	...
	const thinkingLevel = newThread
		? newThreadThinking
		: (tab?.thinkingLevel ?? "medium");
	...
	const error = newThread
		? newThreadError
		: (tab?.error ?? globalError ?? null);

	const activeModel = newThread ? newThreadModel : (tab?.activeModel ?? null);
	const activeCwd = newThread
		? (newThreadCwd ?? groups[0]?.path ?? null)
		: (tab?.cwd ?? null);

	const activeTitle = activePath
		? (groups.flatMap((g) => g.sessions).find((s) => s.path === activePath)
				?.title ?? "Session")
		: activeKey
			? "New Agent"
			: null;
	...
	const respondToUiRequest = useCallback(
		async (response: SessionUiResponse) => {
			const key = tabsRef.current[activeIdxRef.current]?.sessionKey;
			if (!key || !window.pi) return;
			try {
				await window.pi.respondToUiRequest(key, response);
				patchTab(key, (t) => ({
					...t,
					uiRequests: t.uiRequests.filter((r) => r.id !== response.id),
				}));
			} catch (err) {
				patchTab(key, (t) => ({
					...t,
