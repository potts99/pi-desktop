...
import type { SessionEvent, SessionState, TranscriptMessage } from "../../shared/types.ts";
...
describe("useSessions abort isolation", () => {
  it("aborting one streaming session leaves the other streaming", async () => {
    ...
    // Open two sessions.
    await act(async () => { await result.current.openSession({ path: "/s/a.jsonl" }); });
    await act(async () => { await result.current.openSession({ path: "/s/b.jsonl" }); });

    const keyA = result.current.tabs[0].sessionKey;
    const keyB = result.current.tabs[1].sessionKey;
