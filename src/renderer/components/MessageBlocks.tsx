    ...
    <div className="msg-wrap">
      <div className={`msg msg-${msg.role}`}>
        {msg.blocks.map((b, i) => {
          ...
          if (b.kind === "toolCall") {
            const args = b.args as { path?: string; edits?: { oldText: string; newText: string }[] };
            return (
              <Collapsible key={i} summary={toolCallSummary(b)}>
                {b.name === "edit" && Array.isArray(args.edits)
                  ...
                  : <pre className="tool-raw">{JSON.stringify(b.args, null, 2)}</pre>}
              </Collapsible>
            );
          }
      ...
      {msg.role === "user" && msg.id && (
        <div className="msg-actions">
          <button title="Fork from here" onClick={() => onFork?.(msg.id!)}>Branch</button>
        </div>
      )}
    </div>
  );
...
function Collapsible({
  ...
  return (
    <div className={`tool-block${error ? " tool-error" : ""}`}>
      <button className="tool-head" onClick={() => setOpen((o) => !o)} title={open ? "Collapse" : "Expand"}>
...
function resultSummary(b: Extract<TranscriptMessage["blocks"][number], { kind: "toolResult" }>): string {
...
function toolCallSummary(b: { name: string; args: unknown }): string {
  ...
  if (b.name === "bash") return `bash "${truncate(String(a.command ?? ""), 60)}"`;
  if (b.name === "read") return `read "${truncate(String(a.path ?? ""), 80)}"`;
  if (b.name === "grep") return `grep "${truncate(String(a.pattern ?? ""), 40)}"`;
...
function renderToolResult(b: Extract<TranscriptMessage["blocks"][number], { kind: "toolResult" }>) {
