...
export interface AdvisorRuntimeOptions {
  ...
  createClient: (options: { cliPath: string; cwd?: string; args: string[]; env: Record<string, string> }) => AdvisorClient;
}

export function createAdvisorState(): AdvisorState {
  return { client: null, spawnedFor: null, cursor: 0, recentAdvice: [], consecutive: 0, reviewing: false };
}
...
export async function reviewAdvisor(options: AdvisorRuntimeOptions): Promise<void> {
  ...
  try {
    ...
    const advisorClient = await getAdvisorClient(options);
    const idle = waitForIdle(advisorClient);
    await advisorClient.prompt(prompt);
    await idle;

    const parsed = parseAdvisorResponse(await advisorClient.getLastAssistantText());
    ...
    const workerState = await worker.getState().catch(() => ({ isStreaming: false }));
    if (workerState.isStreaming) await worker.steer(wrappedAdvice);
    else await worker.prompt(wrappedAdvice);
    state.consecutive += 1;
  } finally {
    state.reviewing = false;
  }
...
async function syncCursor(state: AdvisorState, worker: WorkerClient): Promise<void> {
  const { entries } = await worker.getEntries().catch(() => ({ entries: [] as SessionEntry[] }));
  state.cursor = entries.length;
}

async function getAdvisorClient(options: AdvisorRuntimeOptions): Promise<AdvisorClient> {
  ...
  const client = options.createClient({
    ...
    args: [
      ...
      "--append-system-prompt", advisorSystemPrompt(config.instructions),
    ],
    env: options.env,
  });
  await client.start();
...
export function parseAdvisorResponse(text: string | null): { severity: AdvisorSeverity; note: string } {
...
export function wrapAdvisory(severity: Exclude<AdvisorSeverity, "none">, note: string): string {
...
function messageText(message: AgentMessage): string {
  ...
  return message.content
    .map((content) => {
      ...
      if (content.type === "toolCall") return `${content.name}\n${JSON.stringify(content.arguments, null, 2)}`;
      return "";
    })
    .join("\n")
    .trim();
...
function advisorSystemPrompt(instructions: string): string {
  ...
  return trimmedInstructions ? `${base}\n\nAdditional project-specific review priorities:\n${trimmedInstructions}` : base;
}
