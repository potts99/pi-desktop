...
function entry(role: AgentMessage["role"], text: string): SessionEntry {
  return { type: "message", message: { role, content: text }, id: Math.random().toString(36) } as unknown as SessionEntry;
}
...
class FakeAdvisor implements AdvisorClient {
  ...
  onEvent(listener: (event: AgentEvent) => void): () => void {
    ...
    return () => { this.listeners = this.listeners.filter((current) => current !== listener); };
  }
  async prompt(message: string): Promise<void> {
    this.prompts.push(message);
    setTimeout(() => this.listeners.forEach((listener) => listener({ type: "agent_end" } as AgentEvent)), 0);
  }
  async getLastAssistantText(): Promise<string | null> { return this.response; }
}

class FakeWorker implements WorkerClient {
  deliveries: Array<{ method: "prompt" | "steer" | "followUp"; message: string }> = [];
  streaming = false;

  constructor(public entries: SessionEntry[] = []) {}

  async getEntries(): Promise<{ entries: SessionEntry[] }> { return { entries: this.entries }; }
  async getState(): Promise<{ isStreaming?: boolean }> { return { isStreaming: this.streaming }; }
  async prompt(message: string): Promise<void> { this.deliveries.push({ method: "prompt", message }); }
  async steer(message: string): Promise<void> { this.deliveries.push({ method: "steer", message }); }
  async followUp(message: string): Promise<void> { this.deliveries.push({ method: "followUp", message }); }
}
...
describe("advisor-runtime", () => {
  ...
  it("caps consecutive interrupting advice until user input", async () => {
    ...
    await review(state, worker, advisor, config({ maxConsecutive: 1 }));
    expect(worker.deliveries.map((delivery) => delivery.method)).toEqual(["prompt", "followUp"]);
  ...
  it("tracks cursor and filters advisory messages", async () => {
    ...
    expect(formatEntriesForAdvisor([entry("user", advisory), entry("assistant", "Fresh")])).toBe("## assistant\nFresh");
  ...
  it("escapes advisory XML", () => {
    expect(wrapAdvisory("blocker", "Use <x> & \"quotes\"")).toContain("Use &lt;x&gt; &amp; &quot;quotes&quot;");
  });
